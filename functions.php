   ...
   if ($stmt = $mysqli->prepare("SELECT idutente, username, password, salt, ruolo FROM utenti WHERE email = ? LIMIT 1")) {
      ...
      if($stmt->num_rows == 1) { // se l'utente esiste
         ...
         if($db_password == $password) { // Verifica che la password memorizzata nel database corrisponda alla password fornita dall'utente.
            ...
            $now = time();
            $mysqli->query("INSERT INTO tentativilogin (user_id, time) VALUES ('$user_id', '$now')");
            return false;
         }
      }
      } else {
...
function checkbrute($user_id, $mysqli) {
   ...
   if ($stmt = $mysqli->prepare("SELECT time FROM tentativilogin WHERE user_id = ? AND time > '$valid_attempts'")) {
