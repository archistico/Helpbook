...
function stampeListaTabella() {
  try {
    ...
    $sql = 'SELECT stampe.*, libri.*, casaeditrice.casaeditrice, soggetti.denominazione FROM stampe '
...
function stampeListaAnnoTabella($anno) {
  try {
    ...
    */
    ...
    $sql = "SELECT SUM(stampe.stampaquantita) AS totalequantita, SUM(stampe.stampacosto) AS totalecosto, SUM(stampe.stampaspedizione) AS totalespedizione, SUM(stampe.stampaiva) AS totaleiva, "
