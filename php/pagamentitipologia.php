...
function pagamentiTipologiaSelectByID($fkpagamentotipologia) {
    try {
        include 'config.php';
        $db = new PDO("mysql:host=" . $dbhost . ";dbname=" . $dbname, $dbuser, $dbpswd);
        $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
        ...
        foreach ($result as $row) {
            $row = get_object_vars($row);
            print "<option value='" . $row['idpagamentotipologia'] . "' ".($row['idpagamentotipologia']==$fkpagamentotipologia?'selected':'').">" . $row['pagamentotipologia'] . "</option>\n";
        }
        // chiude il database
        $db = NULL;
    } catch (PDOException $e) {
