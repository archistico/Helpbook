        ...
        foreach ($result as $row) {
            ...
            switch ($row['codice']) {
                ...
                case 'RI':
                    ...
                    break;
                case 'PR':
                    print "<span class='badge bg-gray'>" . $row['anno'] . "-" . $row['codice'] . "-" . $num_padded . "</span>\n";
                    break;
                default:
            ...
            */
            ...
            if(($row['codice']!=='DT') && ($row['codice']!=='PR')){
                if($row['pagata']) {
...
function movimentoPagatoByID($idmovimento) {
    try {
        include 'config.php';
        $db = new PDO("mysql:host=" . $dbhost . ";dbname=" . $dbname, $dbuser, $dbpswd);
        $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
        ...
        $result = $db->query('SELECT movimenti.pagata '
            . 'FROM movimenti '
            . 'WHERE movimenti.cancellato=0 AND movimenti.idmovimento='.$idmovimento);
    ...
    try {
        ...
        foreach ($result as $row) {
            ...
            if(($row['codice']!=='DT') && ($row['codice']!=='PR')){
                ...
                print "<td><i class = 'fa fa-fw fa-minus' style = 'margin-left: 5px'></i></td>\n";
            }
