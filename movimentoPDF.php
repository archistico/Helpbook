...
<div class="container">
    ...
    for($paginacorrente = 1; $paginacorrente <= $pagineTotali; $paginacorrente++) {
        ...
        for($contatore = ($paginacorrente-1)*$maxrighe+1; $contatore <= $paginacorrente*$maxrighe; $contatore++){
            ...
            $pdf->Cell(15,6,number_format($lista[$contatore-1]->prezzo, 2, ',', ' ').EURO,1,0,'R');
            ...
            // importo scontato
            $pdf->Cell(20,6,number_format($lista[$contatore-1]->quantita*(1-$lista[$contatore-1]->sconto/100)*$lista[$contatore-1]->prezzo, 2, ',', ' ').EURO,1,0,'R');
            $pdf->ln();
        ...
        $pdf->Cell(20,5,number_format($imponibilecarta, 2, ',', ' ').EURO,1,0,'R');
        $pdf->Cell(18,5,number_format($totaleivacarta, 2, ',', ' ').EURO,1,0,'R');$pdf->ln();

        // IVA EBOOK
        $pdf->SetXY(0+$mx+$fondoFirmeX+2,$fondoY+7.5+5+$my);
        $pdf->SetFont('Arial','',8);
        $pdf->Cell(22,5,"Ebook: ".$aliquotaIVAebook."%",1,0,'C');
        $pdf->Cell(20,5,number_format($imponibileebook, 2, ',', ' ').EURO,1,0,'R');
        $pdf->Cell(18,5,number_format($totaleivaebook, 2, ',', ' ').EURO,1,0,'R');$pdf->ln();

        // IVA ALTRO
        $pdf->SetXY(0+$mx+$fondoFirmeX+2,$fondoY+7.5+5+5+$my);
        $pdf->SetFont('Arial','',8);
        $pdf->Cell(22,5,"Altro: ".$aliquotaIVAaltro."%",1,0,'C');
        $pdf->Cell(20,5,number_format($imponibilealtro, 2, ',', ' ').EURO,1,0,'R');
        $pdf->Cell(18,5,number_format($totaleivaaltro, 2, ',', ' ').EURO,1,0,'R');$pdf->ln();
        ...
        $pdf->Cell(30,7.5,number_format($totalenonscontato, 2, ',', ' ').EURO,1,0,'R');
        $pdf->ln();
        $pdf->SetXY(0+$mx+$fondoFirmeX+$fondoPagamentoX,$fondoY+7.5+$my);
        $pdf->SetFont('Arial','',8);
        $pdf->Cell(50,5,"SCONTO TOTALE ",1,0,'R');
        $pdf->Cell(30,5,number_format($totalesconto, 2, ',', ' ').EURO,1,0,'R');
        $pdf->ln();
        $pdf->SetXY(0+$mx+$fondoFirmeX+$fondoPagamentoX,$fondoY+7.5+5+$my);
        $pdf->SetFont('Arial','',8);
        $pdf->Cell(50,5,"IVA TOTALE ",1,0,'R');
        $pdf->Cell(30,5,number_format($totaleivaebook+$totaleivaaltro, 2, ',', ' ').EURO,1,0,'R');
        $pdf->ln();
        $pdf->SetXY(0+$mx+$fondoFirmeX+$fondoPagamentoX,$fondoY+7.5+5+5+$my);
        $pdf->SetFont('Arial','',8);
        $pdf->Cell(50,5,"IMPORTO SCONTATO TOTALE ",1,0,'R');
        $pdf->Cell(30,5,number_format($totalenonscontato-$totalesconto, 2, ',', ' ').EURO,1,0,'R');
        $pdf->ln();
        $pdf->SetXY(0+$mx+$fondoFirmeX+$fondoPagamentoX,$fondoY+7.5+5+5+5+$my);
        $pdf->SetFont('Arial','',8);
        $pdf->Cell(50,7.5,"SPEDIZIONE ".number_format($fat->spesespedizione, 2, ',', ' ').EURO." SCONTO ".$fat->scontospedizione."%",1,0,'R');
        $pdf->Cell(30,7.5,number_format($fat->spesespedizione*(1-$fat->scontospedizione/100), 2, ',', ' ').EURO,1,0,'R');
        $pdf->ln();
        ...
        $pdf->SetFont('Arial','B',14);
        $pdf->Cell(30,15,number_format($totalenonscontato-$totalesconto+($fat->spesespedizione*(1-$fat->scontospedizione/100)), 2, ',', ' ').EURO,1,0,'R');
        $pdf->ln();
